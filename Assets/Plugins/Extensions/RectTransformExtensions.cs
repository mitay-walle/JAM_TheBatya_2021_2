...
namespace Plugins.Extensions
{
    ...
    {
        public static void CloneAnchorsFrom(this RectTransform target, RectTransform sourceX, RectTransform sourceY,
            bool setDirty = false)
        {
            target.sizeDelta = new Vector2(sourceX.sizeDelta.x, sourceY.sizeDelta.y);
            target.pivot = new Vector2(sourceX.pivot.x, sourceY.pivot.y);
            target.anchoredPosition = new Vector2(sourceX.anchoredPosition.x, sourceY.anchoredPosition.y);
            target.anchorMin = new Vector2(sourceX.anchorMin.x, sourceY.anchorMin.y);
            target.anchorMax = new Vector2(sourceX.anchorMax.x, sourceY.anchorMax.y);
            target.offsetMin = new Vector2(sourceX.offsetMin.x, sourceY.offsetMin.y);
            target.offsetMax = new Vector2(sourceX.offsetMax.x, sourceY.offsetMax.y);
            target.anchoredPosition3D = new Vector2(sourceX.anchoredPosition3D.x, sourceY.anchoredPosition3D.y);
            target.localPosition = new Vector2(sourceX.localPosition.x, sourceY.localPosition.y);
            target.localRotation = sourceX.localRotation;
            target.localScale = new Vector2(sourceX.localScale.x, sourceY.localScale.y);

#if UNITY_EDITOR
            if (!Application.isPlaying && setDirty) EditorUtility.SetDirty(target);
#endif
        }

        public static void CloneAnchorsFrom(this RectTransform target, RectTransform source, bool setDirty = false)
        {
        ...
        [Shortcut("RectTransform/Snap to Parent", KeyCode.F, ShortcutModifiers.Alt)]
        public static void SnapToParentShortcut()
        {
            foreach (GameObject gameObject in Selection.gameObjects)
            {
                Undo.RecordObject(gameObject.transform,"Snap to Parent");
                (gameObject.transform as RectTransform).SnapToParent();
...
#endif
        public static bool IsRectTransformInsideSreen(RectTransform rectTransform)
        {
            bool isInside = false;
        ...
        public static void SnapToParent(this Transform rect)
        {
            SnapToParent(rect as RectTransform);
        }
